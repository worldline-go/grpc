...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/worldline-go/grpc/health/v1"
	http "net/http"
	strings "strings"
)
...
var (
	healthServiceDescriptor     = v1.File_health_v1_health_proto.Services().ByName("Health")
	healthCheckMethodDescriptor = healthServiceDescriptor.Methods().ByName("Check")
	healthListMethodDescriptor  = healthServiceDescriptor.Methods().ByName("List")
...
func NewHealthClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) HealthClient {
	...
	return &healthClient{
		...
		list: connect.NewClient[v1.HealthListRequest, v1.HealthListResponse](
			...
			connect.WithSchema(healthListMethodDescriptor),
			connect.WithClientOptions(opts...),
		),
